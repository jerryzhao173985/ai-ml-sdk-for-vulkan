...
In this tutorial, we will generate a sample PyTorch file with a single MaxPool2D operation
to demonstrate each step of the end-to-end workflow.

1. Run the following python script to create a PyTorch model for a single MaxPool2D operation, a NumPy
file used as model input, and use ExecuTorch to convert the model to TOSA FlatBuffers:
...
and model input :code:`input-0.npy`.
...
you need a scenario specification in the form of a JSON file. Pass the VGF file generated in the preceding
step to the VGF Dump Tool:
...
The output from the scenario is produced as a file named :code:`output-0.npy` as specified in scenario.json.
